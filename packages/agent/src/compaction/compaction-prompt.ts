// Copy from https://github.com/vercel/eve/blob/main/packages/eve/src/harness/compaction-prompt.ts
import type { ModelMessage } from "ai";

import { estimateTokens } from "../utils/token-estimate";

export const COMPACTION_CHECKPOINT_MARKER =
...
const DEGRADED_TEXT_LIMIT = 2000;
...
function degradeOversizedTranscript(
  ...
  for (let index = 0; index < entries.length && excessTokens > 0; index += 1) {
    ...
    const degraded = renderCompactionMessageContent(
      message,
      DEGRADED_TEXT_LIMIT
    );
...
function formatCompactionTranscript(
  messages: readonly CompactionTranscriptEntry[]
): string {
...
export const TRANSCRIPT_PAYLOAD_LIMIT = 2000;
...
function renderTranscriptToolCall(
  ...
  const limit =
    conversationTextLimit === undefined
      ? TRANSCRIPT_PAYLOAD_LIMIT
      : COMPACT_PAYLOAD_LIMIT;
...
function renderTranscriptToolResult(
  ...
  return output
    ? `Tool ${part.toolName} ${status} ${output}`
...
function contentToolOutputParts(
  value: unknown
...
function renderToolCall(
  ...
  return input
    ? `Called ${part.toolName} with ${input}`
...
// this shape.
function renderAttachedFileStub(
