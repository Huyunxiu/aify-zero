...
const COMPACTION_SUMMARY_RESERVE_TOKENS = 2048;
...
export function shouldCompact(
  ...
  return (
    messages.length > 0 &&
    getInputTokenCount(messages, config) + COMPACTION_PROMPT_OVERHEAD_TOKENS >
      config.threshold
  );
}
...
type CompactionHeuristic = (
  input: CompactionHeuristicInput
) => CompactionHeuristicOutcome;
 ...
 * entry here.
 */
const COMPACTION_HEURISTICS: readonly CompactionHeuristic[] = [
  toolResultCapHeuristic,
];
 ...
 */
function toolResultCapHeuristic(
  input: CompactionHeuristicInput
): CompactionHeuristicOutcome {
...
function evaluateThreshold(
  ...
  readonly type: "over-limit" | "within-limit";
} {
  const overhead =
    ruler === "should-compact" ? COMPACTION_PROMPT_OVERHEAD_TOKENS : 0;
...
export async function compactMessages(
  ...
  const { conversation, previousCheckpoint } =
    extractPreviousCheckpoint(messages);
  const recentConfig = forceSummary
  ...
  while (true) {
    ...
    if (
      evaluateThreshold(stripped, config, "estimate").type === "within-limit" ||
      keep === 0
    ) {
      return stripped;
    }
...
const CAPPED_RESULT_ANNOTATION =
  "[Truncated by aify0: tool result reduced during context compaction. Re-run the tool if you need the full output.]";
...
function capToolResults(messages: readonly ModelMessage[]): ModelMessage[] {
  return messages.map((message) => {
    ...
    const content = message.content.map((part) => {
      ...
      const output = stubContentOutputFileParts(
        part.output
...
function withResumptionGuard(
  ...
  const alreadyKept =
    replay !== undefined &&
    messages.some(
      (message) => message.role === "user" && message.content === replay.content
    );
 ...
 * verbatim.
 */
function keepNonToolResultMessages(
  messages: readonly ModelMessage[]
): ModelMessage[] {
...
function assistantMessageText(message: ModelMessage): string {
  ...
  return message.content
    .filter(
      (part): part is Extract<ModelMessageContentPart, { type: "text" }> =>
        part.type === "text"
    )
    .map((part) => part.text)
    .join("")
