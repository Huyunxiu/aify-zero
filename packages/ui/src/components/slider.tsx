...
function ValueDisplay({
    ...
    if (editingIndex === index) {
      return (
        <span className="inline-grid text-[13px]">
          ...
          <span
            className="col-start-1 row-start-1 invisible text-base"
            aria-hidden="true"
          >
            {label ? `${label}: ` : ""}
            {formatValue(max)}
          ...
          <span className="col-start-1 row-start-1 flex items-center gap-1">
            ...
            <input
              ...
              className={cn(
                "w-[5ch] bg-transparent text-foreground outline-none border-b border-border text-center text-base rounded-lg",
              )}
            />
          </span>
        </span>
  ...
  return (
    <span
      className={cn(
        "inline-grid shrink-0 text-[13px] leading-none text-muted-foreground transition-[font-variation-settings] duration-100 text-base",
        "tabular-nums"
      )}
    >
      {/* Invisible ghost — reserves width of widest possible value */}
      <span
        className="col-start-1 row-start-1 invisible whitespace-nowrap text-base"
        aria-hidden="true"
      >
        {widestValue}
      </span>
...
function TooltipValue({ value, formatValue, motionX }: TooltipValueProps) {
  ...
  return (
    ...
    >
      <span
        className={cn(
          "text-[12px] text-background tabular-nums whitespace-nowrap bg-foreground px-2 py-1 text-base rounded-lg",
        )}
      >
        {formatValue(value)}
      </span>
    ...
    const renderVisualThumb = (index: number) => {
      ...
      return (
        ...
        >
          ...
          {/* Focus ring */}
          <motion.span
            className="absolute rounded-full border border-[color:var(--ring,#6B97FF)] pointer-events-none"
            initial={false}
            animate={{
    ...
    return (
      ...
      >
        ...
        <AnimatePresence>
          {hoverPreview && showHoverTooltip && !isPressed && (
            ...
            >
              <span
                className={cn(
                  "text-[13px] text-background tabular-nums whitespace-nowrap bg-foreground px-2 py-1 rounded-lg",
                )}
              >
                {formatValue(hoverPreview.snappedValue)}
              </span>
        ...
        <motion.div
          ref={mergedRef}
          className={cn(
            "relative w-full h-8 select-none touch-none border border-border overflow-hidden outline-offset-2 rounded-lg",
            variant === "scrubber"
        ...
        >
          ...
          {variant === "pips" && (
            <motion.div
              className="absolute left-0 top-0 bottom-0 pointer-events-none rounded-[8px] z-[3]"
              style={{
          ...
          {variant === "scrubber" && (
            <motion.div
              className="absolute left-0 top-0 bottom-0 pointer-events-none rounded-[8px]"
              style={{
...
const Slider = forwardRef<HTMLDivElement, SliderProps>(
  ({ size = "default", variant = "pips", ...props }, ref) => {
    const resolved = size;
